...
photon::photon()
{
   ...
   // compton_ratio=(int) ( RAND_MAX*muC/muTotal);
    ...
    rstep=fabs(sampleL)/cellDiv;
    muPEdr=(muTotal - muC)*rstep; // make the muTotal consistent
    muCdr=muTotal*rstep; //muTotal now
}
...
int photon::propagatePass(double theta0)
        // pass through, brutal force version
{
r0=random()/(RAND_MAX+1.0);
if(r0<muPEdr) return(0);
